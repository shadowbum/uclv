...
class Client(object):
    def __init__(self, user, passw, proxy_decrypt):
        self.username = user
        self.password = passw
        self.proxy = None
        if proxy_decrypt:
        	self.proxy = proxy_decrypt
    ...
    def login(self):
        ...
        resp2 = self.session.post(login, data=payload, proxies=self.proxy)
        counter = 0
    ...
    def upload_file(self, file,user_id,progressfunc,args):
        try:
            ...
            resp2 = self.session.post(post_file_url,data=monitor, proxies=self.proxy)
            print(resp2.text)
            of.close()
